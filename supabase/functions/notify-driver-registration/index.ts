...
async function sendSMS(phone: string, message: string) {
  if (!COMTELE_API_KEY) {
    console.error("COMTELE_API_KEY not configured");
    return false;
  }
...
async function extractDocumentData(imageUrls: string[]): Promise<any> {
  if (!LOVABLE_API_KEY) {
  ...
  try {
    const content = [
      {
        type: "text",
        text: `Analise as imagens de documentos enviadas e extraia as seguintes informações em JSON:
          ...
          <tr>
            <td style="padding: 32px;">
              ...
              <p style="margin: 0 0 24px 0; color: #6b7280; font-size: 15px; line-height: 1.6;">
                Finalizou o cadastro e está aguardando sua análise para começar a trabalhar.
              </p>
              ...
              <div style="background-color: #f9fafb; border-radius: 12px; overflow: hidden; border: 1px solid #e5e7eb;">
                <div style="background-color: #7c3aed; padding: 12px 16px;">
                  <h3 style="margin: 0; color: #ffffff; font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">
                    📋 Dados Extraídos por IA
                  </h3>
                </div>
                <table width="100%" cellpadding="0" cellspacing="0">
                  ${dataRows || '<tr><td style="padding: 16px; color: #6b7280; text-align: center;">Nenhum dado extraído automaticamente</td></tr>'}
                </table>
              </div>
              
              <!-- CTA Button -->
              <div style="text-align: center; margin-top: 32px;">
                <a href="#" style="display: inline-block; background: linear-gradient(135deg, #7c3aed 0%, #a855f7 100%); color: #ffffff; text-decoration: none; padding: 16px 48px; border-radius: 12px; font-weight: 700; font-size: 16px; box-shadow: 0 4px 14px rgba(124, 58, 237, 0.4);">
                  Analisar Cadastro
                </a>
              </div>
              
              <p style="margin: 24px 0 0 0; color: #9ca3af; font-size: 13px; text-align: center;">
                Acesse o painel administrativo para aprovar ou rejeitar este motorista.
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="background-color: #f9fafb; padding: 24px 32px; border-top: 1px solid #e5e7eb;">
              <p style="margin: 0; color: #9ca3af; font-size: 12px; text-align: center;">
                Este é um email automático do sistema Bibi Motos.<br>
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!driver_id || !franchise_id) {
      throw new Error("driver_id and franchise_id are required");
    }

    // Fetch driver data with profile
    ...
    // Fetch franchise data with owner profile
    ...
    const documentUrls = [
      driver.cnh_front_url,
      driver.cnh_back_url,
      driver.motorcycle_photo_url,
      driver.motorcycle_plate_photo_url,
      driver.insurance_document_url,
    ].filter(Boolean);

    const extractedData = await extractDocumentData(documentUrls);
    console.log("Extracted data:", extractedData);

    // Create approval request record
    const { error: approvalError } = await supabase
      .from("driver_approval_requests")
      .insert({
        driver_id,
        franchise_id,
    ...
    // Send SMS notification to franchise owner
    if (ownerPhone) {
      const smsMessage = `🏍️ BIBI MOTOS: Novo motorista ${driverName} aguardando aprovação na sua franquia. Acesse o painel para analisar.`;
      await sendSMS(ownerPhone, smsMessage);
      console.log("SMS sent to franchise owner");
    }

    // Generate and log email HTML (would be sent via email service)
    const emailHTML = generateEmailHTML(driverName, extractedData, franchiseName);
    console.log("Email HTML generated for:", ownerEmail);

    // For now, we'll store the email content in analytics for debugging
